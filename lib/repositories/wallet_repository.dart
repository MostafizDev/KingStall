...
class WalletRepository {
  Future<WalletBalanceResponse> getBalance() async {
    final response =
        await http.get(Uri.parse("${AppConfig.BASE_URL}/wallet/balance/${user_id.value}"),headers: { "Authorization": "Bearer ${access_token.value}"},);
  ...
  Future<WalletRechargeResponse> getRechargeList({int page = 1 }) async {
    final response =
        await http.get(Uri.parse("${AppConfig.BASE_URL}/wallet/history/${user_id.value}?page=${page}"),headers: { "Authorization": "Bearer ${access_token.value}"},);
